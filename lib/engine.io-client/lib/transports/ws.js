...
const debug = require('debug')('@parkyang/mp-socket.io-clent:')
...
class HyogaWebSocket extends EventEmitter {
  ...
  addSocketEventListeners() {
    ...
    this._socket.onMessage(data => {
      // 支付宝小程序中使用socketTask：data = {type: 'onSocketTaskMessage', engineType: 1, data: {…}, _preventDefault: false}
      isAlipay ? this.onmessage(data.data, data) : this.onmessage(data)
    })
  }
  ...
  send(data) {
    ...
    if (this.readyState === HyogaWebSocket.OPEN) {
      this._socket.send({ data: /^([^,]+),$/.test(data) ? data.replace(/^([^,]+),$/, '$1') : data })
    }
  }
...
function createConnection(options) {
  const socketTask = nameSpace.connectSocket({
    complete: ()=> {},
    // 支付宝小程序时必须启用 socketTask
    multiple: isAlipay || undefined,
    ...options,
  });
